...
[CmdletBinding()]
param(
  [ValidateSet("info", "build", "build-frontend", "build-backend", "build-auth", "build-api", "tar", "gzip", "build-tar-gzip", "tar-frontend-gzip", "tar-api-gzip", "tar-api-deps-gzip", "tar-all-gzip", "tar-runtime-deps-gzip", "multi-oci-gzip", "vps-frontend-arm64-tar", "vps-backend-arm64-tar", "vps-auth-arm64-tar", "vps-frontend-arm64-gzip", "vps-backend-arm64-gzip", "vps-auth-arm64-gzip", "load-tar", "load-gzip", "list")]
  [string]$Action = "info",
...
function Get-ImageConfig {
  ...
  return [pscustomobject]@{
    ...
    AuthImage     = "$registry/surb-auth:$imageTag"
    RuntimeImages = @(
      "postgres:16-alpine",
      "redis:7-alpine"
    )
...
function Build-Images {
  ...
  Build-SelectedImages -Config $Config -Names @("frontend", "backend", "auth")
}
...
function Build-SelectedImages {
  ...
  try {
    ...
    if ($Names -contains "auth") {
      & docker build -f apps/auth/Dockerfile --target production -t $Config.AuthImage .
      if ($LASTEXITCODE -ne 0) { throw "Build do auth falhou com codigo $LASTEXITCODE" }
    }
  }
  finally {
    Pop-Location
...
function Build-VpsFrontendArm64Tar {
  param($Config)

  Build-VpsArm64Tar -Config $Config -Name "frontend" -Dockerfile "apps/frontend/Dockerfile" -BuildArgs (Get-FrontendBuildArgs $Config)
}

function Build-VpsBackendArm64Tar {
  param($Config)

  Build-VpsArm64Tar -Config $Config -Name "backend" -Dockerfile "apps/backend/Dockerfile"
}

function Build-VpsAuthArm64Tar {
  param($Config)

  Build-VpsArm64Tar -Config $Config -Name "auth" -Dockerfile "apps/auth/Dockerfile"
}

function Build-VpsArm64Tar {
  param(
    $Config,
    [string]$Name,
    [string]$Dockerfile,
  ...
  Ensure-OutputDir
  $vpsImage = "surb/${Name}:production"
  $vpsPlatform = "linux/arm64"
  $vpsTar = Join-Path $OutputDir "surb-$Name-production-arm64.tar"

  Write-Step "Criando imagem $Name para VPS ARM64"
  Write-Host "Imagem: $vpsImage"
  Write-Host "Plataforma: $vpsPlatform"
  Write-Host "Saida: $vpsTar"
  ...
  try {
    & docker buildx build `
      ...
      --target production `
      --tag $vpsImage `
      --output "type=docker,dest=$vpsTar" `
    @($BuildArgs) `
      -f $Dockerfile `
      .

    if ($LASTEXITCODE -ne 0) {
      throw "Build do $Name VPS ARM64 falhou com codigo $LASTEXITCODE"
    }
  }
  finally {
    Pop-Location
  ...
  Write-Host "Gerado: $vpsTar" -ForegroundColor Green
  Write-Host "Na VPS: docker load -i /tmp/surb-$Name-production-arm64.tar" -ForegroundColor Yellow
}

function Compress-VpsArm64TarGzip {
  param(
    [string]$Name
  )

  Ensure-OutputDir
  $source = Join-Path $OutputDir "surb-$Name-production-arm64.tar"
  $destination = "$source.gz"

  if (-not (Test-Path $source)) {
    throw "Arquivo .tar nao encontrado: $source"
  }

  Write-Step "Compactando imagem $Name ARM64 para .tar.gz"
  Write-Host "Origem: $source"
  Write-Host "Destino: $destination"
  Compress-FileToGzip -SourcePath $source -DestinationPath $destination
}
...
function Main {
  ...
  switch ($Action) {
    ...
    "build-auth" {
      Test-Docker
      Show-Info $config
      Build-SelectedImages -Config $config -Names @("auth")
    }

    "build-api" {
      Test-Docker
      Show-Info $config
      Build-SelectedImages -Config $config -Names @("backend", "auth")
    }
    ...
    "vps-frontend-arm64-tar" {
      Test-Buildx
      Show-Info $config
      Build-VpsFrontendArm64Tar $config
    }

    "vps-backend-arm64-tar" {
      Test-Buildx
      Show-Info $config
      Build-VpsBackendArm64Tar $config
    }

    "vps-auth-arm64-tar" {
      Test-Buildx
      Show-Info $config
      Build-VpsAuthArm64Tar $config
    }

    "vps-frontend-arm64-gzip" {
      Show-Info $config
      Compress-VpsArm64TarGzip -Name "frontend"
    }

    "vps-backend-arm64-gzip" {
      Show-Info $config
      Compress-VpsArm64TarGzip -Name "backend"
    }

    "vps-auth-arm64-gzip" {
      Show-Info $config
      Compress-VpsArm64TarGzip -Name "auth"
    }
